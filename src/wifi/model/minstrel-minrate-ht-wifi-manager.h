...
#ifndef MINSTREL_MINRATE_HT_WIFI_MANAGER_H
#define MINSTREL_MINRATE_HT_WIFI_MANAGER_H
...
struct MinstrelMinRateHtWifiRemoteStation;
 ...
 */
typedef std::vector<struct _HtRateInfo> _HtMinstrelRate;
...
struct _GroupInfo
{
  ...
  _HtMinstrelRate m_minstrelTable; ///< Information about rates member in this group
 ...
 */
typedef std::vector<struct _GroupInfo> _McsGroup;

/**
 * Data structure for a Sample Rate table
 * A vector of a vector uint32_t
 */
typedef std::vector<std::vector<uint32_t> > _HtSampleRate;
 ...
 */
class MinstrelMinRateHtWifiManager : public WifiRemoteStationManager
{
public:
  static TypeId GetTypeId (void);
  MinstrelMinRateHtWifiManager ();
  virtual ~MinstrelMinRateHtWifiManager ();
...
private:
  ...
  uint32_t maptoRealTxRateIndex(uint32_t computedRateIndex);
  ...
  void UpdateRetry (MinstrelMinRateHtWifiRemoteStation *station);

  /// getting the next sample from Sample Table
  uint32_t GetNextSample (MinstrelMinRateHtWifiRemoteStation *station);

  /// find a rate to use from Minstrel Table
  uint32_t FindRate (MinstrelMinRateHtWifiRemoteStation *station);

  /// updating the Minstrel Table every 1/10 seconds
  void UpdateStats (MinstrelMinRateHtWifiRemoteStation *station);

  /// initialize Minstrel Table
  void RateInit (MinstrelMinRateHtWifiRemoteStation *station);

  /// initialize Sample Table
  void InitSampleTable (MinstrelMinRateHtWifiRemoteStation *station);

  /// printing Sample Table
  void PrintSampleTable (MinstrelMinRateHtWifiRemoteStation *station);

  /// printing Minstrel Table
  void PrintTable (MinstrelMinRateHtWifiRemoteStation *station);

  void CheckInit (MinstrelMinRateHtWifiRemoteStation *station);  ///< check for initializations
  ...
  uint8_t GetStreams (uint32_t groupId, MinstrelMinRateHtWifiRemoteStation *station);
...
#endif /* MINSTREL_MINRATE_HT_WIFI_MANAGER_H */
