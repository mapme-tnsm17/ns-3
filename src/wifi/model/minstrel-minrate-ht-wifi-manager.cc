...
#include "minstrel-minrate-ht-wifi-manager.h"
...
namespace ns3 {
  ...
  NS_LOG_COMPONENT_DEFINE ("MinstrelMinRateHtWifiManager");

struct MinstrelMinRateHtWifiRemoteStation : public WifiRemoteStation
{
  void DisposeStation ();
 ...
 // MinstrelRate m_minstrelTable;  ///< minstrel table
  _HtSampleRate m_sampleTable;  ///< sample table
  _McsGroup m_mcsTable; ///< MCS groups table
...
void
MinstrelMinRateHtWifiRemoteStation::DisposeStation ()
{
  std::vector<std::vector<uint32_t> >().swap(m_sampleTable);
  for (uint8_t j=0; j< m_mcsTable.size();j++)
    std::vector<struct _HtRateInfo>().swap(m_mcsTable[j].m_minstrelTable);
  std::vector<struct _GroupInfo> ().swap(m_mcsTable);
  
}

NS_OBJECT_ENSURE_REGISTERED (MinstrelMinRateHtWifiManager);

TypeId
MinstrelMinRateHtWifiManager::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::MinstrelMinRateHtWifiManager")
    .SetParent<WifiRemoteStationManager> ()
    .AddConstructor<MinstrelMinRateHtWifiManager> ()
    .AddAttribute ("UpdateStatistics",
                   "The interval between updating statistics table ",
                   TimeValue (Seconds (0.1)),
                   MakeTimeAccessor (&MinstrelMinRateHtWifiManager::m_updateStats),
                   MakeTimeChecker ())
    .AddAttribute ("LookAroundRate",
                   "the percentage to try other rates",
                   DoubleValue (10),
                   MakeDoubleAccessor (&MinstrelMinRateHtWifiManager::m_lookAroundRate),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("EWMA",
                   "EWMA level",
                   DoubleValue (75),
                   MakeDoubleAccessor (&MinstrelMinRateHtWifiManager::m_ewmaLevel),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("SegmentSize",
                   "The largest allowable segment size packet",
                   DoubleValue (6000),
                   MakeDoubleAccessor (&MinstrelMinRateHtWifiManager::m_segmentSize),
                   MakeDoubleChecker <double> ())
    .AddAttribute ("SampleColumn",
                   "The number of columns used for sampling",
                   DoubleValue (10),
                   MakeDoubleAccessor (&MinstrelMinRateHtWifiManager::m_sampleCol),
                   MakeDoubleChecker <double> ())
    .AddAttribute ("PacketLength",
                   "The packet length used for calculating mode TxTime",
                   DoubleValue (65536),
                   MakeDoubleAccessor (&MinstrelMinRateHtWifiManager::m_pktLen),
                   MakeDoubleChecker <double> ())
    .AddTraceSource ("RateChange",
                     "The transmission rate has changed",
                     MakeTraceSourceAccessor (&MinstrelMinRateHtWifiManager::m_rateChange),
                     "ns3::MinstrelMinRateHtWifiManager::RateChangeTracedCallback")
     .AddAttribute ("MinRate",
                   "index of minimal rate",
                   UintegerValue (0),
                   MakeUintegerAccessor (&MinstrelMinRateHtWifiManager::m_minRate),
                   MakeUintegerChecker<uint32_t> ())
  ;
  return tid;
}

MinstrelMinRateHtWifiManager::MinstrelMinRateHtWifiManager ()
{
  m_nsupported = 0;
}
...
void
MinstrelMinRateHtWifiManager::SetupPhy (Ptr<WifiPhy> phy)
{
  //8
...
Time
MinstrelMinRateHtWifiManager::GetCalcTxTime (WifiMode mode) const
{
...
void
MinstrelMinRateHtWifiManager::AddCalcTxTime (WifiMode mode, Time t)
{
  m_calcTxTime.push_back (std::make_pair (t, mode));
}

WifiRemoteStation *
MinstrelMinRateHtWifiManager::DoCreateStation (void) const
{
  MinstrelMinRateHtWifiRemoteStation *station = new MinstrelMinRateHtWifiRemoteStation ();
...
void
MinstrelMinRateHtWifiManager::CheckInit (MinstrelMinRateHtWifiRemoteStation *station)
{
  if (!station->m_initialized && GetNMcsSupported (station) > 1)
    {
      ...
      m_nGroups=1;
      station->m_mcsTable = _McsGroup (m_nGroups);
      station->m_sampleTable = _HtSampleRate ((8-m_minRate), std::vector<uint32_t> (m_sampleCol));
...
void
MinstrelMinRateHtWifiManager::DoReportRxOk (WifiRemoteStation *st,
...
void
MinstrelMinRateHtWifiManager::DoReportRtsFailed (WifiRemoteStation *st)
{
  MinstrelMinRateHtWifiRemoteStation *station = (MinstrelMinRateHtWifiRemoteStation *)st;
...
void
MinstrelMinRateHtWifiManager::DoReportRtsOk (WifiRemoteStation *st, double ctsSnr, WifiMode ctsMode, double rtsSnr)
{
  NS_LOG_DEBUG ("self=" << st << " rts ok");
}

void
MinstrelMinRateHtWifiManager::DoReportFinalRtsFailed (WifiRemoteStation *st)
{
  MinstrelMinRateHtWifiRemoteStation *station = (MinstrelMinRateHtWifiRemoteStation *)st;
...
uint32_t  
MinstrelMinRateHtWifiManager::GetRateId(uint32_t rate)
{
  uint32_t id;
  id = rate % (8-m_minRate);
  return id;
...
uint32_t  
MinstrelMinRateHtWifiManager::GetGroupId(uint32_t rate, WifiRemoteStation *st, uint8_t txstreams)
{
  uint32_t id;
...
void
MinstrelMinRateHtWifiManager::DoReportDataFailed (WifiRemoteStation *st)
{
  MinstrelMinRateHtWifiRemoteStation *station = (MinstrelMinRateHtWifiRemoteStation *)st;
  /**
   *
...
void
MinstrelMinRateHtWifiManager::DoReportDataOk (WifiRemoteStation *st,
...
void
MinstrelMinRateHtWifiManager::DoReportFinalDataFailed (WifiRemoteStation *st)
{
  MinstrelMinRateHtWifiRemoteStation *station = (MinstrelMinRateHtWifiRemoteStation *) st;
...
void
MinstrelMinRateHtWifiManager::UpdateRetry (MinstrelMinRateHtWifiRemoteStation *station)
{
...
WifiTxVector
MinstrelMinRateHtWifiManager::DoGetDataTxVector (WifiRemoteStation *st,
                                    uint32_t size)
{
  MinstrelMinRateHtWifiRemoteStation *station = (MinstrelMinRateHtWifiRemoteStation *) st;
  station->m_txrate=station->m_txrateToUse;
  bool NotInIntializationPhase = true;
  if (!station->m_isSampling)
  {
  ...
  if (!station->m_initialized)
    {
      ...
      if (!station->m_initialized)
        {
             station->m_txrate = 0;
             NotInIntializationPhase = false;
         }
      else
         {
           /// start the rate at half way
           station->m_txrate = (m_nsupported-m_minRate) / 2;
         }
    }
...
//   station->m_isNewPacketSent=true;
  ...
  return WifiTxVector (GetMcsSupported (station, NotInIntializationPhase * maptoRealTxRateIndex(station->m_txrate)), GetDefaultTxPowerLevel (), st->m_slrc, GetShortGuardInterval (st), 1, 0, GetChannelWidth (st), GetAggregation (st), false);
}

uint32_t 
MinstrelMinRateHtWifiManager::maptoRealTxRateIndex(uint32_t computedRateIndex)
{
    return m_minRate + computedRateIndex;
}

WifiTxVector
MinstrelMinRateHtWifiManager::DoGetRtsTxVector (WifiRemoteStation *st)
{
  MinstrelMinRateHtWifiRemoteStation *station = (MinstrelMinRateHtWifiRemoteStation *) st;
    station->m_txrate=station->m_txrateToUse;
  NS_LOG_DEBUG ("DoGetRtsMode m_txrate=" << station->m_txrate);
       ...
       //****************
//   station->m_isNewPacketSent=true;
  return WifiTxVector (GetMcsSupported (station, maptoRealTxRateIndex(0)), GetDefaultTxPowerLevel (), st->m_ssrc, GetShortGuardInterval (st), 1, 0, GetChannelWidth (st), GetAggregation (st), false);//TODO: not sure RTS rate should comply with minstrel rate adaptation
}

bool
MinstrelMinRateHtWifiManager::DoNeedDataRetransmission (WifiRemoteStation *st, Ptr<const Packet> packet, bool normally)
{
  MinstrelMinRateHtWifiRemoteStation *station = (MinstrelMinRateHtWifiRemoteStation *)st;
...
bool
MinstrelMinRateHtWifiManager::IsLowLatency (void) const
{
  return true;
}
uint8_t
MinstrelMinRateHtWifiManager::GetStreams (uint32_t groupId, MinstrelMinRateHtWifiRemoteStation *station)
{
  uint8_t nstreams = 1;
...
uint32_t
MinstrelMinRateHtWifiManager::GetNextSample (MinstrelMinRateHtWifiRemoteStation *station)
{
  ...
  if (station->m_mcsTable[station->m_sampleGroup].m_index >= (7-m_minRate))
    {
...
uint32_t
MinstrelMinRateHtWifiManager::FindRate (MinstrelMinRateHtWifiRemoteStation *station)
{
      ...
      if (idx != station->m_maxTpRate && idx != station->m_txrate)
        {
          ...
          /// error check
          if (idx >= m_nsupported-m_minRate) //TODO: not SURE it is the right way to change the check
            {
...
uint32_t
MinstrelMinRateHtWifiManager::GetTxRate(uint32_t groupid, uint32_t index)
{
  uint32_t rate;
  //Group 0 for LGI group 1 for SGI
  //rate = (groupid*(8-m_minRate))+ index;
  rate=index%(8-m_minRate);//to avoid MCS_index >7 to happen 
  return rate;
}
void
MinstrelMinRateHtWifiManager::UpdateStats (MinstrelMinRateHtWifiRemoteStation *station)
{
  ...
  {
  for (uint32_t i = 0; i < (8-m_minRate); i++) 
    {
...
void
MinstrelMinRateHtWifiManager::RateInit (MinstrelMinRateHtWifiRemoteStation *station)
{
  NS_LOG_DEBUG ("RateInit=" << station);
   ...
   for (uint32_t j = 0; j < m_nGroups; j++)
    {
      //should include MCS
      station->m_mcsTable[j].m_minstrelTable = _HtMinstrelRate (m_nsupported/m_nGroups);
...
void
MinstrelMinRateHtWifiManager::InitSampleTable (MinstrelMinRateHtWifiRemoteStation *station)
{
  ...
  uint32_t numSampleRates = (8-m_minRate);
      ...
      for (uint32_t i = 0; i < numSampleRates; i++ )
        {
          ...
          while (station->m_sampleTable[newIndex][col] != 0)
            {
              newIndex = (newIndex + 1) % (8-m_minRate);
            }
...
void
MinstrelMinRateHtWifiManager::PrintSampleTable (MinstrelMinRateHtWifiRemoteStation *station)
{
...
void
MinstrelMinRateHtWifiManager::PrintTable (MinstrelMinRateHtWifiRemoteStation *station)
{
