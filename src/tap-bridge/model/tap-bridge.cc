...
#define INFRASTRUCTURE_WIFI_PATCH 1
#endif
  ...
  if (pid == 0)
  ...
  else
    {
      ...
      if (m_mode == USE_LOCAL || m_mode == USE_BRIDGE)
	    ...
	    //  std::cout<<"!!!!!!!!!!on node "<<m_node->GetId()<<", set bridged ns3 dev mac="<< learnedMac<<"\n";
  ...
  if (m_mode == USE_LOCAL)
    {
	 ...
	 // std::cout<<"uuuuuuuu Learned MacAddr is " << learnedMac << ": setting ns-3 device to use this address on node "<<m_node->GetId()<<"\n";
          m_ns3AddressRewritten = true;
        }
      // 
   ...
   //   std::cout<<"send to bridged device by use_local on "<<m_node->GetId()<<", to="<<Mac48Address::ConvertFrom (dst)<<"\n";
  ...
  PacketType packetType)
{
  ...
  //    std::cout<<"******************tap bridge see a packet,protocol="<<protocol<<"\n";
  ...
  //std::cout<<">>>>>>>>>>>>>>>>tap bridge send the packet, from="<<header.GetSource ()<<",to="<<header.GetDestination ()<<"\n";
  return true;
}
