                          ...
                          const Address &to, NetDevice::PacketType packetType)
{
  ...
  NS_LOG_LOGIC ("Packet from " << from << " received on node " << 
                m_node->GetId ());
//std::cout<<"RECEIVED: Packet from " << from << " received on node " << 
           //     m_node->GetId ()<<"\n";
...
//std::cout<<"send "<<packet << source << destination << uint32_t (protocol) << route<<"\n";
  Ipv4Header ipHeader;
...
//std::cout<<"Forwarding logic for node: " << m_node->GetId ()<<"\n";
  // Forwarding
